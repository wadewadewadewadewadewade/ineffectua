...
import { addPainLogLocation } from '../../middleware/PainLogMiddleware';
import { ScrollView } from 'react-native-gesture-handler';

type Props = {
  addNewPainLocation: (painlogLocation: PainLogLocation, onComplete: () => void) => void
}
...
export const PainLog = ({
  ...
  const addLocation = (e: GestureResponderEvent, ref: React.Component<SvgProps, any, any> | null, isBack: boolean = false) => {
    if (ref) {
      let width = -1;
      if (ref.props.width) {
        if (typeof ref.props.width === 'number') {
          width = ref.props.width
        } else if (typeof ref.props.width === 'string') {
          width = parseInt(ref.props.width, 10)
        }
      }
      let height = -1;
      if (ref.props.height) {
        if (typeof ref.props.height === 'number') {
          height = ref.props.height
        } else if (typeof ref.props.height === 'string') {
          height = parseInt(ref.props.height, 10)
        }
      }
      console.log(e.nativeEvent.locationX, width, e.nativeEvent.locationY, height)
    }
  }
  return (
    <ScrollView contentContainerStyle={{marginVertical:'auto'}}>
      <View style={styles.container}>
        <Svg ref={(f) => figureFrontRef = f} x="0px" y="0px" viewBox="290 0 420 1000" enable-background="new 290 0 420 1000" style={styles.figure}>
